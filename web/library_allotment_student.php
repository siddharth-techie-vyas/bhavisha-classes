<div class="card-header"><h4>Library Book Allotment (Student)</h4></div>
        ...
        <div class="col-md-3">
            ...
            <div class="col-sm-3">
				...
				<select class='form-control js-states' name="user_id">
                    <option disabled="disabled" selected="selected">-- Select --</option>
                    <?php 
                        $student=$student->viewall($_SESSION['syear'],$_SESSION['branch']);
                        foreach($student as $row=>$value)
                        {
                            $batch = $teacher->get_one_batch($student[$row]['batchid']);
                            echo "<option value='".$student[$row]['id']."'>".$student[$row]['uname']." (".$batch[0]['batch_name'].")</option>";
                        }
                    ?>
                </select>
